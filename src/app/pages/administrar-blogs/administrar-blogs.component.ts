...
@Component({
  ...
  styleUrls: ['./administrar-blogs.component.css']
})
export class AdministrarBlogsComponent {
  private blogService = inject(BlogService);
  private accessoService = inject(AccessoService);
  public listaBlog: MatTableDataSource<blog> = new MatTableDataSource<blog>();
  public displayedColumns: string[] = ['Titulo', 'Fecha de publicación', 'Autor', 'Categoria', 'Acciones'];
  ...
  editarBlog(id: number) {
    this.router.navigate([`editar-blog/${id}`]);
  }
    ...
    if (confirm('¿Estás seguro de que deseas eliminar este blog?')) {
      this.blogService.eliminarBlog(id).subscribe({
        next: () => {
          ...
          this.cargarBlogs(); // Recargar la lista de blogs después de eliminar
        },
        error: (error) => {
